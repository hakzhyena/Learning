...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
    sort(keyboards.begin(), keyboards.end(), std::greater<>());
    sort(drives.begin(), drives.end(), std::greater<>());
    for (std::vector<int>::iterator it = keyboards.begin(); it != keyboards.end(); it++) 
    {
        for (std::vector<int>::iterator it1 = drives.begin(); it1 != drives.end(); it1++) 
        {
            if ((*it + *it1) < b)
                return (*it + *it1);
        }
